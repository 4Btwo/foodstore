...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (user) navigate(ROLE_REDIRECT[user.role] ?? '/dashboard', { replace: true })
  }, [user, navigate])
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (msg.includes('invalid-credential') || msg.includes('wrong-password')) setError('E-mail ou senha incorretos.')
      else if (msg.includes('too-many-requests')) setError('Muitas tentativas. Aguarde alguns minutos.')
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-sm">
        ...
        <p className="mt-6 text-center text-xs text-gray-400">FoodStore • Sistema de gestão para restaurantes</p>
      </div>
    </div>
  )
}
